// main.js – Updated full version with modern full player, playlist sidebar, and single-line category tabs
// Changes:
//  - Full-player playlist is moved into a dedicated .full-playlist-box (above the controls) inside #full-player-right
//  - Category tabs are placed at the top of that playlist box and styled to be on one line
//  - repositionFullPlaylistContainer() updated to place nodes into right panel (not controls container)
//  - Safe guards ensure missing DOM nodes are created non-invasively
//  - Existing behaviors preserved (play, highlight, queue, favorites, modals, etc.)
...
let dvToggleBtn = document.getElementById('dv-toggle-btn');
let discView = document.getElementById('disc-view');
let visualizerView = document.getElementById('visualizer-view');
let fullVolumeBar = document.getElementById('full-volume-bar');

// Quick actions in bottom player
const qaFavoriteBtn = document.getElementById('qa-favorite-btn');
const qaQueueBtn = document.getElementById('qa-queue-btn');
const qaDownloadBtn = document.getElementById('qa-download-btn');
const qaPlaylistBtn = document.getElementById('qa-playlist-btn');
...
let showingVisualizer = false;

/* Ensure minimal full-player UI exists so CSS/JS has nodes to work with */
function ensureMinimalFullPlayerUI() {
    // Create floating open-player icon if missing
    if (!openPlayerIcon) {
        const btn = document.createElement('button');
        btn.id = 'open-player-icon';
        btn.setAttribute('aria-label', 'Open full player');
    ...
    if (!animatedPlayerOverlay) {
        const overlay = document.createElement('div');
        overlay.id = 'animated-player-overlay';
        // basic structure so CSS can apply
        overlay.innerHTML = `
            <div id="full-player-content">
                <div id="full-player-layout">
                    <div id="full-player-hero-row">
                        <div id="full-player-left"></div>
                        <div id="full-player-right"></div>
                    </div>
                    <div id="full-player-controls-container"></div>
                </div>
            </div>
        `;
        document.body.appendChild(overlay);
        animatedPlayerOverlay = overlay;
    }

    // spinning disc + cover
    if (!spinningDisc) {
        const disc = document.createElement('div');
        disc.id = 'spinning-disc';
        const center = document.createElement('div');
        center.id = 'disc-center';
        const img = document.createElement('img');
        img.id = 'full-player-cover';
        img.alt = 'cover';
        center.appendChild(img);
        disc.appendChild(center);
        // ensure left panel exists
        const left = animatedPlayerOverlay.querySelector('#full-player-left') || document.createElement('div');
        left.id = 'full-player-left';
        left.appendChild(disc);
        animatedPlayerOverlay.querySelector('#full-player-hero-row')?.appendChild(left);
        spinningDisc = disc;
        fullPlayerCover = img;
    } else if (!fullPlayerCover) {
        const existing = spinningDisc.querySelector('#full-player-cover');
        if (existing) fullPlayerCover = existing;
    }

    // close button
    if (!closePlayerBtn) {
        const btn = document.createElement('button');
        btn.id = 'close-player-btn';
    ...
    // full volume bar (only if missing create a DOM node but do not force layout changes)
    if (!fullVolumeBar) {
        const fb = document.createElement('input');
        fb.type = 'range';
        fb.id = 'full-volume-bar';
        fb.min = 0;
        fb.max = 100;
        fb.value = 100;
        // append to full-player-controls-container if present so user can see it
        const controlsContainer = animatedPlayerOverlay.querySelector('#full-player-controls-container');
        if (controlsContainer) {
            const volWrap = document.createElement('div');
            volWrap.id = 'full-volume-control';
            volWrap.appendChild(fb);
            controlsContainer.appendChild(volWrap);
        }
        fullVolumeBar = fb;
    }

    // dv toggle + views placeholders (non-invasive)
    if (!dvToggleBtn) {
    ...
    if (!discView) {
        ...
        dv.id = 'disc-view';
        dv.classList.add('dv-view', 'dv-active');
        animatedPlayerOverlay.appendChild(dv);
        discView = dv;
    }
    if (!visualizerView) {
        const v = document.createElement('div');
        v.id = 'visualizer-view';
        v.classList.add('dv-view');
        animatedPlayerOverlay.appendChild(v);
        visualizerView = v;
    }
}
ensureMinimalFullPlayerUI();
        ...
        listItem.innerHTML = `
            <div class="album-card-inner">
                ...
                <div class="album-cover-wrapper" data-song-id="${song.id}">
                    <img
                        src="${song.coverPath || 'covers/default.jpg'}"
        ...
        li.innerHTML = `
            <div class="album-card-inner">
                <div class="album-cover-wrapper">
                    <img
                        src="${firstSong.coverPath || 'covers/default.jpg'}"
        ...
        li.innerHTML = `
            <div class="album-card-inner">
                <div class="album-cover-wrapper">
                    ${
                        firstSong.coverPath
                            ? `<img src="${firstSong.coverPath}" alt="${artistObj.artist}" class="album-cover">`
...
// --- 9. DISC / VISUALIZER TOGGLE ---
function updateDiscVisualizerView() {
    if (!discView || !visualizerView || !dvToggleBtn) {
        if (discView) discView.classList.add('dv-active');
        if (visualizerView) visualizerView.classList.remove('dv-active');
        return;
    }

    visualizerView.classList.remove('dv-active');
    discView.classList.add('dv-active');

    const animatedAlbumImg = document.getElementById('animated-album');
    if (animatedAlbumImg) animatedAlbumImg.classList.remove('playing');
}

// --- 10. AUDIO-REACTIVE VISUALIZER ---
// (Left intentionally minimal — can be expanded later.)
...
function updateQuickActionLabel(song) {
    const qaLabel = document.getElementById('qa-label');
    if (!qaLabel) return;
    qaLabel.textContent = song ? `🎧 ${song.title}` : '🎧 No Song Playing';
}
...
function playSong(song) {
    ...
    audioPlayer.play().catch(e => {
        console.warn('Play prevented by browser:', e);
        // optional: show UI hint (not implemented here)
    });

    currentSongId = song.id;
    playerTitle && (playerTitle.textContent = song.title);
    playerArtist && (playerArtist.textContent = song.artist);
    if (miniCover) {
        miniCover.src = song.coverPath || 'covers/default.jpg';
        miniCover.classList.remove('hidden');
    }

    if (fullPlayerCover) {
        fullPlayerCover.src = song.coverPath || 'covers/default.jpg';
    }

    animatedTitle && (animatedTitle.textContent = song.title);
    animatedArtist && (animatedArtist.textContent = song.artist);
...
function updateQuickActionsState() {
    if (!currentSongId) {
        if (qaFavoriteBtn) qaFavoriteBtn.classList.add('qa-disabled');
        if (qaQueueBtn) qaQueueBtn.classList.add('qa-disabled');
        if (qaDownloadBtn) qaDownloadBtn.classList.add('qa-disabled');
        if (qaPlaylistBtn) qaPlaylistBtn.classList.add('qa-disabled');
        if (qaFavoriteBtn) qaFavoriteBtn.classList.remove('is-favorite');
        return;
    }

    if (qaFavoriteBtn) qaFavoriteBtn.classList.remove('qa-disabled');
    if (qaQueueBtn) qaQueueBtn.classList.remove('qa-disabled');
    if (qaDownloadBtn) qaDownloadBtn.classList.remove('qa-disabled');
    if (qaPlaylistBtn) qaPlaylistBtn.classList.remove('qa-disabled');

    const song = currentSongs.find(s => s.id === currentSongId);
    if (!song || !qaFavoriteBtn) return;

    if (song.isFavorite) {
        qaFavoriteBtn.classList.add('is-favorite');
    } else {
        qaFavoriteBtn.classList.remove('is-favorite');
    }
}
...
function initApp() {
    ...
    // Disc / visualizer toggle
    if (dvToggleBtn) {
        dvToggleBtn.addEventListener('click', () => {
    ...
    // QUICK ACTIONS: favorite / queue / download / playlist(icon only for now)
    if (qaFavoriteBtn) {
    ...
    if (qaQueueBtn) {
        qaQueueBtn.addEventListener('click', () => {
            if (!currentSongId) return;
            const currentSong = currentSongs.find(s => s.id === currentSongId);
            if (!currentSong) return;

            const lastId = userQueue[userQueue.length - 1];
            if (lastId !== currentSong.id) {
                userQueue.push(currentSong.id);
                saveQueueToStorage();
                updateNowPlayingQueue();
            }
        });
    ...
    if (qaDownloadBtn) {
        qaDownloadBtn.addEventListener('click', () => {
            ...
            const title = 'Download this song?';
            const message = `Do you want to download "${currentSong.title}" to your device?`;
